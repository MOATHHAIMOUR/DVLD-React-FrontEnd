...
import { useReplaceLostLicenseHandler } from "../hooks/useReplaceLostLicenseHandler";

const ReplaceLostLicense = () => {
  /* ────────────── State  ────────────── */
  const {
  ...
  const {
    ...
    handleReplaceLostLicense,
  } = useReplaceLostLicenseHandler();
  ...
  const RenewApplicationTypeInfo = applicationTypes?.data.find(
    (r) =>
      r.applicationTypeId ===
      EnumApplicationType.ReplacementForLostDrivingLicense
  );
  ...
  return (
    <Box className="flex flex-col gap-8 mt-8">
      <ErrorHandler error={ReplaceError} />
        ...
        <Box className="grid grid-cols-2 gap-4">
          <Box>
            <Box className="flex items-center mb-2">
              ...
              <span>{ReplaceResult.applicationId || "[???]"}</span>
            </Box>
            <Box className="flex items-center mb-2">
          ...
          <Box>
            <Box className="flex items-center mb-2">
              ...
              <span>
                {ReplaceResult.ReplacementForLostLicenseId || "[???]"}
              </span>
            </Box>
            <Box className="flex items-center mb-2">
...
export default ReplaceLostLicense;
