...
const FindUser = ({ onFindPerson, isDisabled, isLoading }: IProps) => {
  /* ────────────── state  ────────────── */
  ...
  const handleOnChangeFilterBy = useCallback((FilterByName: string) => {
    const FilterBy = FindUserData.find((f) => f.value.name === FilterByName);
  ...
  function handleChangeQuery(
    ...
    switch (FilterType) {
      ...
      case "string":
        if (isNumber(value)) {
          setError("Value Should be a string");
          return;
        }
        setError(null);
  ...
  const renderFilterBys = useMemo(
    () =>
      FindUserData.map((filterBy, index) => (
